...
export async function createShippingZoneAction(
  ...
  if (!parsed.success) return { ok: false, errorKey: 'validation.failed' };
  if (parsed.data.estimatedDeliveryDaysMin > parsed.data.estimatedDeliveryDaysMax) {
  ...
  try {
    ...
    let finalCode = code;
    let suffix = 1;
    while (await prisma.shippingZone.findUnique({ where: { code: finalCode } })) {
      suffix += 1;
...
export async function updateShippingZoneFullAction(
  ...
  try {
    ...
    if (before.active && !parsed.data.active && before._count.governorateConfigs > 0) {
