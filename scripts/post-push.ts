...
// Order = administrative/CAPMAS order (used as the default `position`).
...
async function main() {
  ...
  // Sprint 11.5 — seed `GovernorateConfig` for all 27 enum codes. This is
  // the new admin-editable surface (deliverable toggle, names, position,
  // zone reassignment) while `GovernorateZone` above stays in lock-step
  // for legacy readers (`lib/shipping/resolve.ts`). `update: {}` preserves
  // admin edits — owner can rename a governorate or toggle deliverable from
  // /admin/settings/governorates and the change survives redeploys.
  for (let i = 0; i < GOVERNORATE_LABELS.length; i++) {
    const entry = GOVERNORATE_LABELS[i]!;
    const mapping = GOVERNORATE_ZONES.find(
      (g) => g.governorate === entry.code,
    );
  ...
  // Default payment mode: LIVE in production, TEST in dev/staging. Owner
  // flips from /admin/settings/payment-methods (gated by admin password).
  const PAYMENT_MODE_KEY = 'payment.mode';
  const existingMode = await prisma.setting.findUnique({
  ...
  // Default Whats360 transport mode: read at runtime by lib/whatsapp.ts so
  // owner can flip to DEV (log-only, no API call) or SANDBOX (Whats360
  // sandbox endpoint, no billable sends) without redeploying. Owner-gated
  // by admin password from /admin/settings/whatsapp.
  const WHATSAPP_MODE_KEY = 'whatsapp.transport';
  const existingWhatsapp = await prisma.setting.findUnique({
