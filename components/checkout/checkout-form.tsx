...
type Props = {
  ...
  /// true; if both are false the form shows a clear "no methods available"
  /// notice.
...
export function CheckoutForm({
  ...
  // Sprint 11.5 — default to whichever method is enabled. If COD is on,
  // prefer it (lower friction); else PAYMOB_CARD; else fall back to COD
  // (form will surface a "no methods enabled" error in the UI).
          ...
          <section className="space-y-4 rounded-xl border border-border bg-paper p-5">
            ...
            !paymentMethodAvailability.paymobCardEnabled ? (
              <div className="rounded-md border border-error/40 bg-error/10 p-3 text-sm text-error">
                {isAr
                  ? 'لا توجد طرق دفع مفعّلة حالياً. تواصل معنا على واتساب لإتمام الطلب.'
                  : 'No payment methods are currently enabled. Please contact us on WhatsApp to complete your order.'}
              </div>
            ) : null}
          </section>
        ) : (
